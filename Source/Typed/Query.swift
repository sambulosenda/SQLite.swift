...
extension QueryType {
    ...
    public var first: Select<Row?> {
        return Select(limit(1, clauses.limit?.offset).expression)
    }
...
extension Connection {
    ...
    // FIXME: use `try!` since this shouldn't fail?
    public func prepare(query: QueryType) throws -> AnySequence<Row> {
        ...
        let columnNames: [String: Int] = try {
            ...
            column: for each in query.clauses.select.columns ?? [Expression<Void>(literal: "*")] {
                var names = split(each.expression.template.characters) { $0 == "." }.map(String.init)
                ...
                if column == "*" {
                    ...
                    if !namespace.isEmpty {
                        ...
                        assertionFailure("no such table: \(namespace)")
                    }
                    for q in queries {
