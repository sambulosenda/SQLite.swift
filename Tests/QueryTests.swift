...
class QueryTests : XCTestCase {
    ...
    func test_select_withStar_resetsSelectClause() {
        AssertSQL("SELECT * FROM \"users\"", users.select(email).select(*))
    }
...
//        AssertSQL("SELECT * FROM \"users\" ORDER BY \"age\" DESC, \"email\" ASC", users.order(age, email.desc).reverse())
    ...
    func test_first_returnsSelectExpressionWithLimit() {
        AssertSQL("SELECT * FROM \"users\" LIMIT 1", users.first)
    }

    func test_first_retainsOffset() {
        AssertSQL("SELECT * FROM \"users\" LIMIT 1 OFFSET 10", users.limit(10, offset: 10).first)
    }
    ...
    func test_subscript_withExpression_returnsNamespacedExpression() {
        ...
        AssertSQL("\"query\".\"data\"", query[data])
        AssertSQL("\"query\".\"dataOptional\"", query[dataOptional])
