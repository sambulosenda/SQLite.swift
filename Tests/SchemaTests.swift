...
class SchemaTests : XCTestCase {
    ...
    func test_drop_compilesDropViewExpression() {
        XCTAssertEqual("DROP VIEW \"view\"", view.drop())
        XCTAssertEqual("DROP VIEW IF EXISTS \"view\"", view.drop(ifExists: true))
    }
    ...
    func test_create_withQuery_compilesCreateTableExpression() {
        XCTAssertEqual(
            ...
            table.create(view.select(int64))
        )
    }
    ...
    func test_create_onView_compilesCreateViewExpression() {
        XCTAssertEqual(
            ...
            view.create(table.select(int64))
        )
        XCTAssertEqual(
            "CREATE TEMPORARY VIEW \"view\" AS SELECT \"int64\" FROM \"table\"",
            view.create(table.select(int64), temporary: true)
        )
        XCTAssertEqual(
            "CREATE VIEW IF NOT EXISTS \"view\" AS SELECT \"int64\" FROM \"table\"",
            view.create(table.select(int64), ifNotExists: true)
        )
        XCTAssertEqual(
            "CREATE TEMPORARY VIEW IF NOT EXISTS \"view\" AS SELECT \"int64\" FROM \"table\"",
            view.create(table.select(int64), temporary: true, ifNotExists: true)
        )
    }
